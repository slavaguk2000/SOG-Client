...
//        });
    ...
    {
        final boolean finalState = visibility;
        final int intVisibility, intNotVisibility;
        if (visibility) {
            intVisibility = View.VISIBLE;
            intNotVisibility = View.INVISIBLE;
            setBlackImage();
        }
        else {
            intVisibility = View.INVISIBLE;
            intNotVisibility = View.VISIBLE;
        }
    ...
    public void connectButtonTriggered(final View v)
    {
        connectToServer();
    }
    public void connectToServer()
    {
        try{
            ...
            connectionTask.execute(params.toArray());
        }catch (Exception e){}
    }
    ...
    {
        final Bitmap blackBitmap = Bitmap.createBitmap(100, 100, Bitmap.Config.ALPHA_8);
        blackBitmap.eraseColor(Color.BLACK);
        Handler handler = new Handler(Looper.getMainLooper());
        handler.post(new Runnable() {
            public void run() {
                songImage.setImageBitmap(blackBitmap);
            }
        });
    }
